...
class TestCaseDAO {
	//@Test를 하겠다. (어노테이션)
	//Junit에서 메소드 이름을 원하는 문자열로 치환해서 보여줌(가독성때문에 쓰는거)
	//timeOut: 응답시간. 메소드를 호출하고 결과를 받기까지의 시간. (웹 디비에 접속 수 데이터를 받아오기까지 위한 시간)
	...
	void testSum() {
		...
		//When
		int result = dao.sum(0, 0); // -> 0 + 0 = 0임
		
		//Then:검증 오류
		assertEquals(11, result);  // 0과 11이 달라서 실행하면 fail 뜸
		
		assertNotEquals(12, result);  // assertNotEquals: 달라야한다  // 실행하면 성공
	}
